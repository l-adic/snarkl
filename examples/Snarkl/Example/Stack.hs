...
type Stack a = TExp (TStack a) (Prime p)

empty_stack :: (Typeable a) => Comp (TStack a)
empty_stack = nil

push_stack :: (Typeable a) => TExp a (Prime p) -> Stack a -> Comp (TStack a)
push_stack p q = cons p q

pop_stack :: (Derive a, Zippable a, Typeable a) => Stack a -> Comp (TStack a)
pop_stack f = tail_list f

top_stack :: (Derive a, Zippable a, Typeable a) => TExp a (Prime p) -> Stack a -> Comp a
